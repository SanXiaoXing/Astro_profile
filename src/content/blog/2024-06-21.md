...
updatedDate: 2025-03-24 00:03:69
description: 每日刷力扣
catalog: true
categories:
  - Leetcode
...
1. **字母异位词的判断**：两个字符串是字母异位词的充要条件是它们的字符排序后结果相同。例如，"eat"和"tea"排序后均为"aet"。

2. **哈希表分组**：使用字典 `groups`，其中键是排序后的字符串，值是该组的原始字符串列表。

3. **遍历字符串数组**：对每个字符串进行排序，生成键，将原字符串添加到对应的组中。

4. **结果提取**：将字典中的所有值（即各组的列表）收集并返回。
...
![result](https://cdn.jsdelivr.net/gh/SanXiaoXing/Image@main/blog/SanXiaoXing_2025-03-24_00-37-17.png)

## [3. 无重复字符的最长子串](https://leetcode.cn/problems/longest-substring-without-repeating-characters/)

### Idea

1. **滑动窗口**：使用两个指针（左指针 `left` 和右指针 `right`）维护一个窗口，窗口内的字符都是唯一的。
    
2. **哈希表记录位置**：通过哈希表（字典）存储每个字符最后一次出现的位置，以便快速判断字符是否重复。
    
3. **动态调整窗口**：当右指针遇到重复字符时，移动左指针到重复字符的下一个位置，确保窗口内字符唯一。
...
![result](https://cdn.jsdelivr.net/gh/SanXiaoXing/Image@main/blog/SanXiaoXing_2025-03-24_00-26-08.png)
# 2024-06-24
